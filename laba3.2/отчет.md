...
Сильный форматтер, составной оператор языка Go (с опциональными точками с запятой,
...
%type <ident> start func_decl func_item var_decl for_decl if_decl func_params expr expr_log 
%type <ident> expr_una expr_decl ident_val type
...
func_decl:
    FUNC_ST ident_val LPAREN func_params RPAREN type LBRACE {
		...
		if (strlen($4) > 0){
            // sprintf("======================");
			$$ = MEM(2 * strlen(indent) + strlen("func") + strlen($2) + strlen($4) + strlen("(){")
             + strlen($6) + 2 + strlen("\n"));
...
```
func m(a int,b int) {a = b;
for i := 0; i < 10; i++ {if a > b {a =  1}} a = 4;
}
```
...
# Вывод
Был реализован сильный форматтер языка Go с помощью генератора синтаксических анализаторов bison